...
def p_if(p):
    '''
    if : JO '(' condition ')' '{' instruction_list '}'
        | JO '(' condition ')' '{' instruction_list '}' ATHVA '{' instruction_list '}'
...
def p_for(p):
    '''
    for : MATE '(' var_assign  ';' condition ';' incrementation  ')' '{' instruction_list '}'
        | MATE '(' var_assign  ';' condition ';' decrementation  ')' '{' instruction_list '}'
        | MATE '(' expression ';' condition ';' incrementation  ')' '{' instruction_list '}'
        | MATE '(' expression ';' condition ';' decrementation  ')' '{' instruction_list '}'
    '''
...
def p_while(p):
    '''
    while : JYA '(' condition ')' '{' instruction_list '}'
...
def p_doWhile(p):
    '''
    doWhile :  KRO '{' instruction_list '}' JYA '(' condition ')'
    '''
    p[0] = (p[1], p[3], p[7])
...
def p_condition_big(p):
    '''
    condition : '(' condition ')' YA '(' condition ')'
              | '(' condition ')' ANE '(' condition ')'
...
def p_condition_medium1(p):
    '''
    condition : condition ANE '(' condition ')'
...
def p_try(p):
    '''
    try :  PRAYAS '{' instruction_list '}' SIVAY '{' instruction_list '}'
        |  PRAYAS '{' instruction_list '}' SIVAY '{' instruction_list '}' AKHIRAN '{' instruction_list '}'
    '''
    if len(p) == 9:
        p[0] = (p[1], p[3], p[5], p[7])
    else:
...
def p_arrfn(p):
    '''
    arrfn : ID '.' JODO '(' expression ')'
          | ID '.' HATAVO '(' expression ')'
          | ID '.' HATAVO '(' ')'
    '''
    if len(p) == 7:
...
def p_func(p):
    '''
    func : BANAVO ID '(' parameter_list ')' '{' instruction_list '}'
         | BANAVO ID '(' ')' '{' instruction_list '}'
    '''
    if(len(p) == 8):
        p[0] = ('banavo', p[2], p[6])
    else:
        p[0] = ('banavo', p[2], p[4], p[7])
...
def p_printing(p):
    '''
    printing : CHHAPO '(' condition ')'
            | CHHAPO '(' incrementation ')'
            | CHHAPO '(' decrementation ')'
            | CHHAPO '(' condition ',' condition ')'
    '''
    if len(p) == 5:
        p[0] = (p[1], p[3])
    else:
